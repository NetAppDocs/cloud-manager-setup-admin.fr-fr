...
La politique IAM présentée ci-dessous fournit les autorisations nécessaires à un connecteur pour gérer les ressources et les processus de votre environnement de cloud public en fonction de votre région AWS.

Lorsque vous créez un connecteur directement à partir de BlueXP, BlueXP applique automatiquement cette stratégie au connecteur.

Si vous déployez le connecteur depuis AWS Marketplace ou si vous installez manuellement le connecteur sur un hôte Linux, vous devrez définir vous-même la stratégie.
